...
router.get('/url', async function(req, res) {
  ...
  //DOMPurify, takes the HTML and cleans it before we move forward to parse it with JSDOM
  const window = new JSDOM('').window; //not sure why I need to do it.
  const DOMPurify = createDOMPurify(window); // create an object that contains all the logic to clean the HTML
  const cleanWebData = DOMPurify.sanitize(webData);
  console.log(cleanWebData);

  //turn the clean HTML into DOM document
  const dom = new JSDOM(cleanWebData);
  console.log(dom);
